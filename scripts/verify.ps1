...
#              uv run alembic current   (expects 0004 (head))
...
#              npx playwright test e2e/golden-path.spec.ts e2e/unicode.spec.ts e2e/segmentation.spec.ts e2e/token-segmentation.spec.ts
#
...
Invoke-VerifyStep 'backend: uv run alembic current (expect 0004 (head))' $ApiRoot @('uv', 'run', 'alembic', 'current')
...
Invoke-VerifyStep 'e2e: golden path + Unicode + M2/M3 segmentation (npx playwright test)' $WebRoot @('npx', 'playwright', 'test', 'e2e/golden-path.spec.ts', 'e2e/unicode.spec.ts', 'e2e/segmentation.spec.ts', 'e2e/token-segmentation.spec.ts')
